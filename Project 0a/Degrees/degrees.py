...
def shortest_path(source, target):
    ...
    """
    # Fix for path of length 0 (source is the target)
    if source == target:
        return []

    # Initialise a QueueFrontier for BFS
    ...
    while True:
        ...
        # Remove next node to expand and add to explored
        ...
        for action, state in neighbors_for_person(curr_node.state):
            if not frontier.contains_state(state) and state not in explored:
                new_node = Node(state=state, parent=curr_node, action=action)

                # Goal check upon adding to frontier for efficiency
